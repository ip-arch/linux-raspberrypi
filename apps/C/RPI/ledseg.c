...
int main(int argc, char **argv)
{
  ...
  *(gpio+10)=0x03C6c390;  /* GPCLR0 turn off all LEDs */
  for(i=0;i<10;i++) {
	*(gpio+10)=DispSel[i&1];  /* GPSET0 turn on LEDs */
	*(gpio+7)=DispSel[1-(i&1)];  /* GPSET0 turn on LEDs */
     for(rep=0;rep<10;rep++) {
  	  *(gpio+10)=pat;  /* GPCLR0 turn off LEDs */
	  pat = Pat[rep] ;
	  *(gpio+7)=pat;  /* GPSET0 turn on LEDs */
	  sleep(1);
	  }
  }
  return 0;

...
static irqreturn_t irq_sw_thread(int irq, void *dev_id)
{
	...
	schedule_delayed_work(&debounce_work, msecs_to_jiffies(20));
	return IRQ_HANDLED;
}

static void debounce_fn(struct work_struct *w)
{
	s64 delay_us = ktime_us_delta(ktime_get(), irq_time);
	int val = gpiod_get_value_cansleep(gpio_sw);

	if (val == 1) {
		pr_info("SW confirmed ON\n");
		count++;
		ledvalue = 1 - ledvalue;
		gpiod_set_value_cansleep(gpio_led, ledvalue);
	}
	pr_info("GPIO interrupt #%d: SW=%d (top->workqueue delay: %lld us)\n",
            count, gpiod_get_value_cansleep(gpio_sw), delay_us);
}
...
static int my_probe(struct platform_device *pdev)
{
    ...
    INIT_DELAYED_WORK(&debounce_work, debounce_fn);
